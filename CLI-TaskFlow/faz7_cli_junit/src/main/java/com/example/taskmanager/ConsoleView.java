...
public class ConsoleView {
    ...
    private void printLoginMenu() {
        ...
        try {
            ...
            switch (choice) {
                ...
                default: printError("Action yaft nashod!");
            }
        } catch (Exception e) {
    ...
    private void printMainMenu() {
        ...
        ConsoleColors.println("  │  8. VSD Kardan (exit with saveLog)", ConsoleColors.ORANGE);
        ConsoleColors.println("  │  0. Khorooj", ConsoleColors.RED);
        ConsoleColors.println("  │", ConsoleColors.PURPLE);
        ConsoleColors.printlnBold("  └────────────────────────────────", ConsoleColors.PURPLE);
    ...
    private void handleSignUp() {
        ...
        try {
            ...
            printSuccess("Sabt nam ba movafaghiyat!");
        } catch (Exception e) {
    ...
    private void handleLogin() {
        ...
        try {
            ...
            loggedInUser = name;
            printSuccess("Vorod ba movafaghiyat!");
        } catch (Exception e) {
            printError("Error dar vorod: " + e.getMessage());
        }
    }
    ...
    private void showAllUsers() {
        ...
        if (karbarHa.isEmpty()) {
            ConsoleColors.println("  Hich karbari vojod nadarad!", ConsoleColors.GRAY);
        } else {
    ...
    private void handleLogout() {
        loggedInUser = null;
        printSuccess("Khoroj ba save data!");
    }
    ...
    private void handleShowAllTasks() {
        ...
        try {
            ...
            if (tasks.isEmpty()) {
                ConsoleColors.println("  Hich taski vojod nadarad!", ConsoleColors.GRAY);
            } else {
                for (Task task : tasks) {
                    printTask(task);
                }
        ...
        try {
            ...
            for (int i = 0; i < pendingTasks.size(); i++) {
                ...
                ConsoleColors.println("  " + (i + 1) + ". " + task.getOnvan() + 
                    " (Olaviat: " + task.getPayvand() + ")", ConsoleColors.LIGHT_BLUE);
            }
    ...
    private void handleFilterTasks() {
        ...
        ConsoleColors.println("  │  4. Task Haye Olaviat Bala (4+)", ConsoleColors.ORANGE);
        ...
        try {
            ...
            switch (choice) {
                ...
                case 0: break;
                default: printError("Action namoafagh!");
            }
        } catch (Exception e) {
    ...
    private void filterHighPriority() {
        try {
            ...
            ConsoleColors.println("\n  Task Haye Olaviat Bala (4+):", ConsoleColors.ORANGE);
            if (tasks.isEmpty()) {
                ConsoleColors.println("  Hich taski nist!", ConsoleColors.GRAY);
            } else {
                tasks.forEach(this::printTask);
    ...
    private void handleVirgool() {
        ConsoleColors.println("\n  Vaghti VSD konid, data zakhire mishavad.", ConsoleColors.GRAY);
    ...
    private void printTask(Task task) {
        ...
        ConsoleColors.println("  " + vaziat + " " + payvandEmoji + " " + task.getOnvan() + 
            " | Olaviat: " + task.getPayvand() + " | Deadline: " + task.getMokalefZaman(), color);
    }
    ...
    private void printTaskWithRank(Task task, int rank) {
        ...
        ConsoleColors.println("  " + rank + ". " + vaziat + " " + payvandEmoji + " " + task.getOnvan() + 
            " | Olaviat: " + task.getPayvand(), ConsoleColors.YELLOW);
    }
